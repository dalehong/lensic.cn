...
<tbody>
    ...
    <tr id="tr_power_<?php echo $data['id']; ?>">
        ...
        <td><?php echo $data['type']==0?'----':$data['school']; ?></td>
        <td><?php echo $data['type']=='1'?'老师':$data['type']==2?'学生':'系统'; ?></td>
        <td><?php echo $data['use_count']; ?> 人使用</td>
        <td><?php if($data['id'] == 1): ?><img src="<?php echo SITE_ADMIN_STATIC; ?>/images/edit_gray.gif" alt="不可编辑" title="不可编辑" /> <img src="<?php echo SITE_ADMIN_STATIC; ?>/images/del_gray.gif" alt="不可删除" title="不可删除" /><?php else: ?><a href="<?php echo site_url('power/group_edit/' . $data['id']); ?>" title="编辑"><img src="<?php echo SITE_ADMIN_STATIC; ?>/images/edit.gif" alt="编辑" /></a> <?php if($data['use_count']): ?><img src="<?php echo SITE_ADMIN_STATIC; ?>/images/del_gray.gif" alt="不可删除" title="该组已在使用，不可删除" /><?php else: ?><a href="javascript:void(0);" onClick="return show_dialog('确定删除该记录吗？', 'del', '<?php echo site_url('power/group_del/' . $data['id']); ?>', 'tr_power_<?php echo $data['id']; ?>');" title="删除"><img src="<?php echo SITE_ADMIN_STATIC; ?>/images/del.gif" alt="删除" /></a><?php endif; ?><?php endif; ?></td>
    </tr>
    <?php endforeach; ?>
</tbody>
</table>
