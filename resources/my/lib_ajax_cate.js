...
function ajax(url, back_fun, send_data, json)
{
	...
	// 第二步：绑定回调函数
	ajax.onreadystatechange = function (){back_ajax(ajax, back_fun, json);};
	// 第三步：建立连接，实现异步传输
 ...
 *	
 *	调用：
 *		<script type="text/javascript" src="lib_ajax_cate.js"></script>
 *		<script type="text/javascript">
 *		// 从顶级分类 0 开始，显示无限级别分类
 *		stepless_classification('show_list_one', 'pid[]', 'index.php?pid=', 0);
 *		
 *		// 从 id=11 数据开始，不显示 id=11 数据项，但显示它的同类和父类
 *		stepless_classification('show_list_two', 'pid[]', 'index.php?pid=', 3, '&id=11');
 *		
 *		// 从 id=11 数据开始，显示 id=11 数据项并选中，同时显示它的同类和父类，但不显示子类
 *		stepless_classification('show_list_three', 'pid[]', 'index.php?pid=', 3, '&id=11', '&type=1');
 *		
 *		// 从 id=11 数据开始，显示 id=11 数据项并选中，同时显示它的同类、父类和子类
 *		stepless_classification('show_list_four', 'pid[]', 'index.php?pid=', 3, '&id=11', '&type=2');
 *		</script>
 *
 */
// 无级分类下拉列表封装
function stepless_classification(obj_id, select_name, ajax_url, pid, id_str, display_type, option_default, select_level)
{
	...
	if(isNaN(parseInt(pid)))
	{
		...
		ajax(ajax_url + pid + (id_str ? id_str : (option_default ? option_default : '')) + (display_type ? display_type : ''), back_stepless_classification, '', {obj_id:obj_id, select_name:select_name, ajax_url:ajax_url, pid:pid, id_str:id_str, display_type:display_type, option_default:option_default, select_level:select_level});
	}
}
...
function back_stepless_classification(ajax, json)
{
	...
	if(json.id_str)
	{
		...
		if(json.display_type && !json.option_default)
		{
			...
			{*/
				...
				json.option_default = typeof(last_id[1]) == 'undefined' ? last_id[0].replace(/\//g, '') : last_id[1];
			/*}*/
		}
	} else {
		obj.appendChild(sel);
	...
	if(json.id_str)
	{
		...
		if(back_data.ppid)
		{
			stepless_classification(json.obj_id, json.select_name, json.ajax_url, back_data.ppid, json.id_str, json.display_type, back_data.pid, json.select_level + 1);
		} else {
			...
			for(var i = 0; i < len; i++)
			{
				...
				{
					stepless_classification(json.obj_id, json.select_name, json.ajax_url, parseInt(this.value), '', json.display_type, json.id_str, parseInt(this.getAttribute('level')) + 1);
				};
			}
			if(json.display_type)
			{
				...
				{
					var last_id = json.id_str.split('=');
					stepless_classification(json.obj_id, json.select_name, json.ajax_url, typeof(last_id[1]) == 'undefined' ? last_id[0].replace(/\//g, '') : last_id[1], '', json.display_type, '', json.select_level + 1);
				}
			}
		}
	} else {
		...
		{
			stepless_classification(json.obj_id, json.select_name, json.ajax_url, parseInt(this.value), json.id_str, json.display_type, json.option_default, json.select_level + 1);
		};
	}
}
...
{
	obj_id = typeof(obj_id) == 'object' ? obj_id : document.getElementById(obj_id);
