...
class RadarAxis extends AxisPrototype {
    ...
    function _StrokeAxisTitle($pos,$aAxisAngle,$title) {
        ...
        if($title=="C型 常规型"){
        	$size=10;
        }else if($title=="E型 事业型"){
        	$size=40;
        }else if($title=="A型 艺术型"){
        	$size=90;
        }else if($title=="I型 调查型"){
        	$size=90;
        }else{
        	$size=$this->scale->world_abs_size;
        }
				if($title=="S型 社会型"){
				  $xt=round(($size+$marg)*0.3+$this->scale->scale_abs[0]); 	
				}else if($title=="R型 实际型"){
				  $xt=round(($size+$marg)*0+$this->scale->scale_abs[0]); 	
				}else{
					$xt=round(($size+$marg)*cos($aAxisAngle)+$this->scale->scale_abs[0]);
				}
