...
class Gszc extends A_Controller
{
    ...
    function user_add()
    {
        ...
        if(strtolower($_SERVER['REQUEST_METHOD']) == 'post')
        {
            ...
            $post['uname']       	= $this->input->post('uname');
            $post['studentid']      = $this->input->post('studentid');
    ...
    function zycp()
    {
        ...
        $where_array=array('mid'=>$this->session->userdata('admin_uid'));
        $view_datas['member'] = $this->m_common->get_one('member',$where_array,'mid,mbti_id,mbti_time,hld_id,hld_time,zyfx_id,zyfx_time,qznl_id,qznl_time,zymyd_id,zymyd_time,studentid');
        if(empty($view_datas['member']['studentid'])){
        	redirect('index/index_info');
        }
    ...
    {	
		...
		switch ($type) {
			...
			case 'zymyd':
				$view_datas['title'] = '恭喜，职业满意度测试提交成功！'; 
				$title="职业满意度";
			break;
			
			case 'grzl':
				$view_datas['title'] = '恭喜，个人资料提交成功！'; 
			break;
		...
		if($member['studentid']=='-1'){  //判断是否为学生 -1职场， 1学生
			if($member['hld_id']>0 && $member['mbti_id']>0){  // && $member['zyfx_id']>0
				$message  = ' 霍兰德报告：'.$this->get_url('hld',$mid,$member['hld_id'],'bg');
				$message .= ' 霍兰德详情：'.$this->get_url('hld',$mid,$member['hld_id'],'xq');	
				$message .= ' MBTI报告：'.$this->get_url('mbti',$mid,$member['mbti_id'],'bg');
				$message .= ' MBTI详情：'.$this->get_url('mbti',$mid,$member['mbti_id'],'xq');
				$message .= ' 职业满意度报告：'.$this->get_url('zymyd',$mid,$member['zymyd_id'],'bg');
				$message .= ' 职业满意度详情：'.$this->get_url('zymyd',$mid,$member['zymyd_id'],'xq');
			...
			if($member['hld_id']>0 && $member['mbti_id']>0){
				$message  = ' 霍兰德报告：'.$this->get_url('hld',$mid,$member['hld_id'],'bg');
				$message .= ' 霍兰德详情：'.$this->get_url('hld',$mid,$member['hld_id'],'xq');	
				$message .= ' MBTI报告：'.$this->get_url('mbti',$mid,$member['mbti_id'],'bg');
				$message .= ' MBTI详情：'.$this->get_url('mbti',$mid,$member['mbti_id'],'xq');
    ...
    {	
		...
		$this->email->to('diana@ccdm.com.cn'); 
    ...
    function gslist()
    {
    	...
    	$admin_type = $this->session->userdata('admin_school');
		if(!empty($admin_type)){
