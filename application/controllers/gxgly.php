 ...
 */
class Gxgly extends A_Controller
{
    ...
    function user()
    {
        ...
        $config['per_page'] = 20; 
        $config['base_url'] = site_url('gxgly/xszl') . '?status=' . $status . ($search_key ? '&search_key=' . $search_key : '');
    ...
    function user_add()
    {
        ...
        //$this->m_gxgly->zu_datas(array('get_count' => TRUE, 'search_key' => $search_key));
        if(strtolower($_SERVER['REQUEST_METHOD']) == 'post')
        {
            ...
            $where_array = array('school' => $this->session->userdata('admin_school'), 'type' => 2);
            $group = $this->m_common->get_one('power_group',$where_array,'id');
    ...
    function xszl()
    {
        $view_datas['title'] = '学生资料';
        $this->check_power('学生资料_gx');
        $view_datas['search_key'] = $search_key = $this->input->get('search_key');
        $view_datas['status'] = $status = $this->input->get('status') ? $status = $this->input->get('status') : 1;
        $this->load->helper('my_page');
        $config['per_page'] = 20; 
        $config['base_url'] = site_url('gxgly/xszl') . '?status=' . $status . ($search_key ? '&search_key=' . $search_key : '');
        $config['total_rows'] = $this->m_gxgly->xszl_datas(array('get_count' => TRUE, 'search_key' => $search_key, 'power_group_id' => $this->power_group_id));
        $view_datas['pages'] = page_links($config);
        $view_datas['datas'] = $this->m_gxgly->xszl_datas(array('limit' => TRUE, 'search_key' => $search_key, 'power_group_id' => $this->power_group_id));
    ...
    function zycp()
    {
        ...
        $this->check_power('职业测评_gx');
        $view_datas['search_key'] = $search_key = $this->input->get('search_key');
        $view_datas['status'] = $status = $this->input->get('status') ? $status = $this->input->get('status') : 1;
        $this->load->helper('my_page');        
        $config['per_page'] = 20; 
        $config['base_url'] = site_url('gxgly/zycp') . '?status=' . $status . ($search_key ? '&search_key=' . $search_key : '');
        $config['total_rows'] = $this->m_gxgly->xszl_datas(array('get_count' => TRUE, 'search_key' => $search_key, 'power_group_id' => $this->power_group_id));
        $view_datas['pages'] = page_links($config);
        $view_datas['datas'] = $this->m_gxgly->xszl_datas(array('limit' => TRUE, 'search_key' => $search_key, 'power_group_id' => $this->power_group_id));
    ...
    function hld()
    {
        ...
        $this->load->view('gxxs/zycp_hld_bg', $view_datas);
    }  
    /**
     * 职业测评之霍兰德__学生填写的详情
     */
    function hld_xq()
    {
        ...
        $view_datas['leixing'] = explode('||', $view_datas['hld']['leixing']);
        $view_datas['jiazhi'] = explode('||', $view_datas['hld']['jiazhi']);
        $this->load->view($this->file_name . 'hld_xq', $view_datas);
    }
    ...
    function mbti_xq()
    {
        ...
        $view_datas['ceping'] = $this->m_common->get_one('cp_mbti',$where_array);        
        $view_datas['mbti'] = $this->m_common->get_one('cp_mbti_result',array('type'=>$view_datas['ceping']['result']));     
        $this->load->view($this->file_name . 'mbti_xq', $view_datas);
    }
    ...
    function qznl()
    {
        $view_datas['title'] = '求职能力测评报告';
        $this->check_power('职业测评_gx');
        $cp_id = $this->uri->segment(3);
        $where_array=array('id'=>$cp_id);        
        $view_datas['ceping'] = $this->m_common->get_one('cp_qznl',$where_array);        
        $this->load->view($this->file_name . 'qznl', $view_datas);
    }
    /**
     * 职业测评之MBTI__学生填写的详情
     */
    ...
    function qznl_xq()
    {
        $view_datas['title'] = '求职能力测评详情';
        $this->check_power('职业测评_gx');
        $cp_id = $this->uri->segment(3);
        $where_array=array('id'=>$cp_id);        
        $view_datas['ceping'] = $this->m_common->get_one('cp_qznl',$where_array);            
        $this->load->view($this->file_name . 'qznl_xq', $view_datas);
    }
    ...
    function cykxx_xq()
    {
        ...
        $where_array=array('id'=>$cp_id);        
        $view_datas['ceping'] = $this->m_common->get_one('cp_cykxx',$where_array);            
        $this->load->view($this->file_name . 'cykxx_xq', $view_datas);
    }
    /**
     * 职业测评之职业分析
     */
    function zyfx()
    {
        $view_datas['title'] = '分析问卷';
        $this->check_power('职业测评_gx');
        $cp_id = $this->uri->segment(3);
        $where_array=array('id'=>$cp_id);           
        $view_datas['fx'] = $this->m_common->get_one('cp_zyfx',$where_array);     
        $this->load->view('gxxs/zycp_zyfx_bg'.SITE_YX, $view_datas);
    }/**
     * 职业测评之职业分析__学生填写的详情
     */
    function zyfx_xq()
    {
        $view_datas['title'] = '分析问卷详情';
        $this->check_power('职业测评_gx');
        $cp_id = $this->uri->segment(3);
        $where_array=array('id'=>$cp_id);           
        $view_datas['fx'] = $this->m_common->get_one('cp_zyfx',$where_array);     
    ...
    function cpbg()
    {
        ...
        //echo $this->db->last_query();
        //$view_datas['xszl'] = $this->m_gxgly->power_datas();
        $this->load->view($this->file_name . 'cpbg', $view_datas);
    }
    ...
    function zjzx()
    {
        ...
        $config['per_page'] = 20; 
        $config['base_url'] = site_url('gxgly/cpbg') . '?status=' . $status . ($search_key ? '&search_key=' . $search_key : '');
        $config['total_rows'] = $this->m_gxgly->zjzx_datas(array('get_count' => TRUE, 'search_key' => $search_key, 'power_group_id' => $this->power_group_id));
        $view_datas['pages'] = page_links($config);
        $view_datas['datas'] = $this->m_gxgly->zjzx_datas(array('limit' => TRUE, 'search_key' => $search_key, 'power_group_id' => $this->power_group_id));
    ...
    function zxfk()
    {
        ...
        $this->check_power('咨询反馈_gx');        
        $view_datas['search_key'] = $search_key = $this->input->get('search_key');
        $this->load->helper('my_page');
        $config['per_page'] = 20; 
        $config['base_url'] = site_url('gxgly/zxfk') . ($search_key ? '?search_key=' . $search_key : '');
        $config['total_rows'] = $this->m_gxgly->zxfk_datas(array('get_count' => TRUE, 'search_key' => $search_key, 'power_group_id' => $this->power_group_id));
        $view_datas['pages'] = page_links($config);
        $view_datas['datas'] = $this->m_gxgly->zxfk_datas(array('limit' => TRUE, 'search_key' => $search_key, 'power_group_id' => $this->power_group_id));
    ...
    function zxbg()
    {
        ...
        $this->load->helper('my_page');
        $config['per_page'] = 20; 
        $config['base_url'] = site_url('gxgly/zxbg') . ($search_key ? '?search_key=' . $search_key : '');
        $config['total_rows'] = $this->m_gxgly->zxbg_datas(array('get_count' => TRUE, 'search_key' => $search_key, 'power_group_id' => $this->power_group_id));
        $view_datas['pages'] = page_links($config);
        $view_datas['datas'] = $this->m_gxgly->zxbg_datas(array('limit' => TRUE, 'search_key' => $search_key, 'power_group_id' => $this->power_group_id));
    ...
    function zxszl()
    {
        ...
        $this->check_power('咨询师资料_gx');        
        $view_datas['search_key'] = $search_key = $this->input->get('search_key');
        $this->load->helper('my_page');
        $config['per_page'] = 20; 
        $config['base_url'] = site_url('gxgly/zxszl') . ($search_key ? '?search_key=' . $search_key : '');
        $config['total_rows'] = $this->m_gxgly->zxszl_datas(array('get_count' => TRUE, 'search_key' => $search_key));
        $view_datas['pages'] = page_links($config);
        $view_datas['datas'] = $this->m_gxgly->zxszl_datas(array('limit' => TRUE, 'search_key' => $search_key));
    ...
    function zxszl_xq()
    {
        ...
        $config['per_page'] = 20; 
        $config['base_url'] = site_url('gxgly/zxszl') . ($search_key ? '?search_key=' . $search_key : '');
        $config['total_rows'] = $this->m_gxgly->zxbg_datas(array('get_count' => TRUE, 'search_key' => $search_key));
        $view_datas['pages'] = page_links($config);
        $view_datas['datas'] = $this->m_gxgly->zxbg_datas(array('limit' => TRUE, 'search_key' => $search_key));
